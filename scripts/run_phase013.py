...
def collect(iterator):
    subs = []; real_by = {m: [] for m in E.MODELS}; pseudo_by = {m: [] for m in E.MODELS}
...
def run_config(cfg, K, datasets, subset, cache_dir, min_pseudo, targets_only=False, clean_pseudo=False,
    ...
    dbeta_rows = []; inter_store = {}; persubject_rows = []; r2_rows = []
    for tag, make_iter in makers.items():
        print(f"\n===== {tag} | {cname} | K={K} | min_pseudo={min_pseudo} =====")
        subs, real_by, pseudo_by, inter = collect(make_iter())
        if not subs:
            print(f"  no usable subjects for {tag}"); continue
        inter_store[tag] = inter
        for m in E.MODELS:
            res = E.contrast_ragged(real_by[m], pseudo_by[m], min_draws=min_pseudo)
            ...
            r2m, r2beta, r2n = E.marginal_r2(inter[m]['feat'], inter[m]['p300'], inter[m]['subject'])
    ...
    if len(inter_store) >= 2:
        ...
        for m in E.MODELS:
            ...
            for tag in tags:
                r = inter_store[tag][m]
                feat += r['feat']; p3 += r['p300']
                subj += [f"{tag}:{s}" for s in r['subject']]; dset += [tag] * len(r['feat'])
...
def main():
    ...
    ap.add_argument('--subset', type=int, default=None, help='limit #subjects (quick test)')
    ...
    ap.add_argument('--cache-dir', default='./phase013_cache', dest='cache_dir')
    ...
    import phase013_erpcore as _ec, phase013_engine as _en
    print("="*70)
    ...
    all_db = []; all_it = []; all_ps = []; all_r2 = []
    for cfg in cfgs:
        db, it = run_config(cfg, a.k, datasets, a.subset, cache_dir, a.min_pseudo, a.targets_only,
                            a.clean_pseudo, a.resample_hz)
        if len(db):
            all_db.append(db)
            if 'persubject' in db.attrs and len(db.attrs['persubject']): all_ps.append(db.attrs['persubject'])
            if 'marginal_r2' in db.attrs and len(db.attrs['marginal_r2']): all_r2.append(db.attrs['marginal_r2'])
        if len(it): all_it.append(it)
    if all_db:
        db = pd.concat(all_db, ignore_index=True)
        p = os.path.join(a.out, f"phase013_dbeta_{suffix}.csv"); db.to_csv(p, index=False)
        print(f"\nWrote {p}\n"); print(db.to_string(index=False))
    if all_it:
        it = pd.concat(all_it, ignore_index=True)
        p = os.path.join(a.out, f"phase013_interaction_{suffix}.csv"); it.to_csv(p, index=False)
        print(f"\nWrote {p}\n"); print(it.to_string(index=False))
    if all_ps:
        ps = pd.concat(all_ps, ignore_index=True)
        p = os.path.join(a.out, f"phase013_persubject_{suffix}.csv"); ps.to_csv(p, index=False)
        print(f"\nWrote {p} ({len(ps)} per-subject rows for participant-level figures)")
    if all_r2:
        r2 = pd.concat(all_r2, ignore_index=True)
        p = os.path.join(a.out, f"phase013_marginal_r2_{suffix}.csv"); r2.to_csv(p, index=False)
